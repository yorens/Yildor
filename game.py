...
class Game:
    ...
    def create_first_menu(self):
        first_menu = menu.Menu("Do you want to...", "Enter the number of your choice: ")
        first_menu.add_menu_item(menu.MenuItem("Start a new game", "c1", 1))
        first_menu.add_menu_item(menu.MenuItem("Exit", "c2", 2))
        return first_menu

    def set_up_players(self):
        player_menu = menu.Menu("", "How many players are playing (please enter a number)? ", True, True)
        player_menu.display()
        num_players_input = player_menu.get_choice_as_int()
        ...
        for i in range(num_players_input):
            player_name = input("Enter the name for player " + str(i + 1) + ": ")
